...
function initParticles() {
  particles = [];
  let particleSize = random(5, 20); // Define the size of each particle
  for (let i = 0; i < cols; i++) {
    for (let j = 0; j < rows; j++) {
    particles.push(new Particle(i * resolution, j * resolution, particleSize));
    }
  }
}
...
function connectParticles() {
  stroke(80); // Black color for the lines
...
class Particle {
  constructor(x, y, size) {
    ...
    this.r = size; // radius or size of the particle
  }
  ...
  separate(particles) {
    let desiredseparation = this.r * 2.1;
    ...
    if (count > 0) {
      steer.div(count);
      steer.normalize();
      steer.mult(2); // Assuming 2 is the max speed
      steer.sub(this.vel);
      steer.limit(0.9); // Assuming 0.1 is the max force
    }
    return steer;
  }
  ...
  update() {
    ...
    this.vel.limit(1.1); // Limiting the velocity to slow down the movement
